...
import killua.dev.aitalk.ui.viewmodels.OpenAIConfigUIIntent
import killua.dev.aitalk.ui.viewmodels.OpenAIConfigViewModel
import killua.dev.aitalk.utils.LocalNavHostController
import kotlinx.coroutines.launch

@OptIn(ExperimentalMaterial3Api::class, ExperimentalAnimationApi::class)
@Composable
fun OpenAIConfigPage() {
    val scrollBehavior = TopAppBarDefaults.exitUntilCollapsedScrollBehavior(rememberTopAppBarState())
    val viewModel: OpenAIConfigViewModel = hiltViewModel()
    val uiState = viewModel.uiState.collectAsStateWithLifecycle()
    val navController = LocalNavHostController.current!!
    val scope = rememberCoroutineScope()
    val context = LocalContext.current
    val parentModel = AIModel.ChatGPT
    ...
    ConfigurationsScaffold(
        ...
        actions = {
            ...
            Button(
                onClick = {
                    scope.launch { viewModel.emitIntent(OpenAIConfigUIIntent.SaveSettings) }
                ...
                Column(
                    ...
                    PreferenceSwitchWithContainer(
                        ...
                        scope.launch {
                            ...
                            viewModel.emitIntent(OpenAIConfigUIIntent.ToggleModelEnabled(newCheckedState))
                        }
                    }
                    Title(title = stringResource(R.string.default_model)) {
                        SingleChoiceSegmentedButtonRow(
                            ...
                            subModels.forEachIndexed { index, subModel ->
                                SegmentedButton(
                                    onClick = { scope.launch { viewModel.emitIntent(OpenAIConfigUIIntent.SelectOption(index)) } },
                    ...
                    Title(title = stringResource(R.string.api_key_settings)) {
                        BaseTextField(
                            ...
                            onValueChange = { scope.launch { viewModel.emitIntent(OpenAIConfigUIIntent.UpdateApiKey(it)) } },
                    ...
                    Title(title = stringResource(R.string.floating_window_system_instruction_title)) {
                        BaseTextField(
                            ...
                            onValueChange = { scope.launch { viewModel.emitIntent(OpenAIConfigUIIntent.UpdateFloatingWindowSystemInstruction(it)) } },
                    ...
                    Title(title = stringResource(R.string.system_instruction)) {
                        BaseTextField(
                            value = uiState.value.openAIConfig.systemInstruction,
                            onValueChange = { scope.launch { viewModel.emitIntent(OpenAIConfigUIIntent.UpdateOpenAISystemInstruction(it)) } },
                    ...
                    Title(title = stringResource(R.string.parameter_settings)) {
                        Slideable(
                            ...
                            value = uiState.value.openAIConfig.temperature.toFloat(),
                            valueRange = 0F..2F,
                            steps = 0,
                            desc = uiState.value.openAIConfig.temperature.toString(),
                            onValueChange = { scope.launch { viewModel.emitIntent(OpenAIConfigUIIntent.UpdateOpenAITemperature(it.toDouble())) } },
                        )
                    }
                }
            }
