...
import killua.dev.aitalk.ui.viewmodels.GeminiConfigViewModel
import killua.dev.aitalk.utils.LocalNavHostController
import kotlinx.coroutines.launch

@OptIn(ExperimentalMaterial3Api::class, ExperimentalAnimationApi::class)
@Composable
fun GeminiConfigPage() {
    val scrollBehavior = TopAppBarDefaults.exitUntilCollapsedScrollBehavior(rememberTopAppBarState())
    val viewModel: GeminiConfigViewModel = hiltViewModel()
    val uiState = viewModel.uiState.collectAsStateWithLifecycle()
    val navController = LocalNavHostController.current!!
    val scope = rememberCoroutineScope()
    val context = LocalContext.current
    val parentModel = AIModel.Gemini
    ...
    ConfigurationsScaffold(
        ...
        actions = {
            ...
            Button(
                onClick = {
                    scope.launch { viewModel.emitIntent(GeminiConfigUIIntent.SaveSettings) }
                ...
                Column(
                    ...
                    PreferenceSwitchWithContainer(
                        ...
                        scope.launch {
                            ...
                            viewModel.emitIntent(GeminiConfigUIIntent.ToggleModelEnabled(newCheckedState))
                        }
                    }
                    Title(title = stringResource(R.string.default_model)) {
                        SingleChoiceSegmentedButtonRow(
                            ...
                            subModels.forEachIndexed { index, subModel ->
                                SegmentedButton(
                                    onClick = { scope.launch { viewModel.emitIntent(GeminiConfigUIIntent.SelectOption(index)) } },
                    ...
                    Title(title = stringResource(R.string.api_key_settings)) {
                        BaseTextField(
                            ...
                            onValueChange = { scope.launch { viewModel.emitIntent(GeminiConfigUIIntent.UpdateApiKey(it)) } },
                    ...
                    Title(title = stringResource(R.string.floating_window_system_instruction_title)) {
                        BaseTextField(
                            ...
                            onValueChange = { scope.launch { viewModel.emitIntent(GeminiConfigUIIntent.UpdateFloatingWindowSystemInstruction(it)) } },
                    ...
                    Title(title = stringResource(R.string.parameter_settings)) {
                        Slideable(
                            ...
                            onValueChange = { scope.launch { viewModel.emitIntent(GeminiConfigUIIntent.UpdateGeminiTemperature(it.toDouble())) } },
                        )
                        Slideable(
                            ...
                            onValueChange = { scope.launch { viewModel.emitIntent(GeminiConfigUIIntent.UpdateGeminiTopP(it.toDouble())) } },
                        )
                        Slideable(
                            ...
                            onValueChange = { scope.launch { viewModel.emitIntent(GeminiConfigUIIntent.UpdateGeminiTopK(it.toInt())) } },
                        )
                        BaseTextField(
                            value = uiState.value.geminiConfig.responseMimeType,
                            onValueChange = { scope.launch { viewModel.emitIntent(GeminiConfigUIIntent.UpdateGeminiResponseMimeType(it)) } },
